...
public class OrderRepositoryImpl implements OrderRepository {
    ...
    public List<OrderVO> getOrderList(OrderSearchVO vo) {
        Session session = this.getCurrentSession();
        try {
            String sql = "select id, delivery_name as deliveryName, delivery_code as deliveryCode, order_code as orderCode, " +
                    "distributor_id as distributorId, account_id as accountId, \n" +
            ...
            if(null != vo.getDistributorId() && vo.getDistributorId() > 0) {
                condition += " and distributor_id = " + vo.getDistributorId();
            }
            if(null!=vo.getOrderIds() && !vo.getOrderIds().isEmpty()) {
                ...
                if(length>1){
                    condition += " and id in "+tmp.substring(0,length-2)+")";
                }else{
                    //报错
                }
            }
            if (null != vo.getOrderCode() && !"".equals(vo.getOrderCode())) {
                condition += " and order_code like '%" + vo.getOrderCode() + "%' ";
            }
            if (null != vo.getStartDate() && !"".equals(vo.getStartDate())) {
                condition += " and order_time &gt;= DATE_FORMAT(" + vo.getStartDate() + ", %Y-%m-%d %H:%i:%s) ";
            }
            if (null != vo.getEndDate() && !"".equals(vo.getEndDate())) {
                condition += " and order_time &lt;= DATE_FORMAT(" + vo.getEndDate() + ", %Y-%m-%d %H:%i:%s) ";
            }
            if (null != vo.getOrderStatus() && 0 != vo.getOrderStatus()) {
                condition += " and status = " + vo.getOrderStatus();
            }
            if (null != vo.getPayStatus() && 0 != vo.getPayStatus()) {
                condition += " and payment_status = " + vo.getPayStatus();
            }
            if (null != vo.getReceiveName() && !"".equals(vo.getReceiveName())) {
                condition += " and receiver_name like '%" + vo.getReceiveName() + "%'";
            }
            if (null != vo.getReceivePhone() && !"".equals(vo.getReceivePhone())) {
                condition += " and receiver_phone like '%" + vo.getReceivePhone() + "%'";
            }
            if(condition!="") {
                condition = condition.substring(4,condition.length());
                condition = " where " + condition;
            }
            sql += condition +  " order by order_time desc";
    ...
    public OOrderEntity selectOrderByKey(Integer id) {
        Session session = this.getCurrentSession();
        return new SelectByPrimarkKey<OOrderEntity>("OOrderEntity", session, id).getData();
    }

    public Integer updateOrder(OOrderEntity entity) {
        Session session = this.getCurrentSession();
        return new Update<OOrderEntity>(session, entity).getData();
    }
}
